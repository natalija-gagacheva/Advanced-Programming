package auditoriski.Prereshi.LogProcessor_Nepominata;
...
/*zadacha bara da se napravat 3 konkretni procesori na logovi (so pomosh na lambda izraci)
toa znaci deka se potrebni da se napravat 3 implementacii na ovoj interface LogProcessor
 Mozhime da gi napravime so lmbda izrazi bidejki ovaj pretstavuva funkciski interface i cuva samo 1 metod*/
interface LogProcessor <T extends ILog>{

    //vrakja procesirani logovi, prima logs
    public ArrayList<T> processLogs(ArrayList<T> logs);
}

class LogSystem <T extends ILog> {

    //ne ni e eksplicitno navedeno kakvi logovi treba da chuvame
    ...
    void printResults() {
        ...
        //zadachata bara da se napravat 3 konkretni procesori na logovi (so pomosh na lambda izraci)
        //gi zemame site logs koi shto imaat nekakov type
        LogProcessor<T> firstLogProcessor = (logs)->logs.stream().filter(log->log.getType().equals("INFO")).collect(Collectors.toCollection(()->new ArrayList<>()));

        LogProcessor<T> secondLogProcessor = (logsList) -> logsList.stream().filter(log->log.getMessage().length()<100).collect(Collectors.toCollection(()->new ArrayList<>()));

        LogProcessor<T> thirdLogProcessor = (logsList)->logsList.stream().sorted((l1,l2)->{
            if(l1.getTimestamp()>l2.getTimestamp())return 1;
            else if (l1.getTimestamp()<l2.getTimestamp())return -1;
            return 0;
...
//ovaa klasa e implementacija na Ilog
